...
with prompt_container:
    ...
    # 4. Assistant Step 2 - Create a Run
        st.toast(body="Thinking...please wait.", icon="⏳")
        with chat_container:
            status2 = st.status(label="⏳ Thinking...please wait.", expanded=False)
    ...
    # 5. Assistant Step 3 - List Messages When Completed
        if run.status == "completed":
            ...
            st.toast(body="Response completed!", icon="✅")
            status2.update(label="✅ Response completed!", expanded=False, state="complete")
